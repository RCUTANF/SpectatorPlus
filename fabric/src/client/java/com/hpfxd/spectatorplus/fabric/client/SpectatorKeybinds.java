...
public class SpectatorKeybinds {
    ...
    private static void selectInMenu(Minecraft mc, UUID uuid) {
        ...
        final SpectatorMenuItem menuItem = Iterables.find(category.getItems(), item -> item instanceof PlayerMenuItem && uuid.equals(mc.getConnection().getListedOnlinePlayers().stream().filter(p -> p.getProfile().name().equals(item.getName().getString())).findFirst().map(p -> p.getProfile().id()).orElse(null)));
